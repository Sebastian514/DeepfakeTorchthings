...
parser.add_argument('-discriminator', type=bool, default=True, help='Use adversarial loss in training loop')
...
for epoch in range(args.n_steps):
    ...
    LOSSA.append(loss_a.data.cpu())
    LOSSB.append(loss_b.data.cpu())
    if epoch % 1000 == 0:
        print(to_print)
        model_state_dict = model.state_dict()
        torch.save(model_state_dict, '{}/{}.pt'.format(args.saved_dir, args.model_name))
X_label = [i for i in range(args.n_steps)]
plt.plot(X_label, LOSSA, label="ironman")
plt.plot(X_label, LOSSB, label="Musk")
plt.xlabel("epoch")
plt.ylabel("loss")
plt.title("Convergence Curve For Non-Adversial Training")
plt.savefig("unadversialtrain2.png")
